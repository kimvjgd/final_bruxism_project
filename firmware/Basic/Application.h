  ...
  while (1)
  {
    if(CURRENT_MODE) {
      if(CURRENT_EMG_VALUE > EMG_THR_VALUE) {
        Serial.println("Exceed EMG Threshold Value");
        CURRENT_MODE = SET_ELEC_STIMUL_MODE();    // Change CURRENT_MODE 1 (EMG) => 0 (E.S.) 
      } else {
        CURRENT_EMG_VALUE = analogRead(SIGNAL_OUT);
        if(DEBUG_MODE) {
          Serial.print("Current EMG Value : ");
          Serial.println(CURRENT_EMG_VALUE);
        }
      }
      vTaskDelay(100 / portTICK_RATE_MS);       // 혹시 에러가 나면 여기를 if 문 밖으로 뺴보자
    }
  }
}
...
void ES_task(void *pvParameter) 
{
  ...
  while (1)
  {
    if(!CURRENT_MODE) {
      ...
      // PULSE가 끝났다.
      if(CUR_PULSE_ORDER > PULSE_PERIOD_CNT) {
        CURRENT_MODE = SET_EMG_MODE();
      }
...
void freeRTOS_setup()
{
  xTaskCreatePinnedToCore(&EMG_task, "EMG_Task", 8192, NULL, 1, NULL, 0);
