...
class SerialPlotter:
    def __init__(self, master):
        ...
        self.master.grid_rowconfigure(4, weight=0)  # 새 행 추가
        self.master.grid_rowconfigure(5, weight=0)  # 새 행 추가
        ...
        self.emg_label = tk.Label(master, text="EMG      :  N/A V", font=label_font)
        self.emg_label.grid(row=4, column=0, sticky="w")
        self.voltage_label = tk.Label(master, text="Voltage  :  N/A V", font=label_font)
        self.voltage_label.grid(row=4, column=1, sticky="w")
        self.current_label = tk.Label(master, text="Current   :  N/A mA", font=label_font)
        self.current_label.grid(row=5, column=0, sticky="w")
        self.power_label = tk.Label(master, text="Power    :  N/A mW", font=label_font)
        self.power_label.grid(row=5, column=1, sticky="w")

        # 경고 메시지 박스 추가
        self.show_warning()

    def show_warning(self):
        messagebox.showwarning("!!!Caution!!!", "Press Stop for recording the data in excel file!")
    ...
    def update_plot(self):
        while not self.data_queue.empty():
            ...
            # 레이블 업데이트
            self.emg_label.config(text=f"EMG      :  {new_y_emg} V")
            self.voltage_label.config(text=f"Voltage  :  {new_y_voltage} V")
            self.current_label.config(text=f"Current   :  {new_y_current} mA")
            self.power_label.config(text=f"Power    :  {new_y_power} mW")
